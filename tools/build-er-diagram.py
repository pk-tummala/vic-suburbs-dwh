...
W, H = 1660, 1000
NAVY = "#1A2B4A"; BLUE = "#3D6BBA"; TEAL = "#1FA39A"; AMBER = "#E0A436"; VIOLET = "#8A6BBF"
INK = "#33415C"; MUTE = "#6B7B96"; LINE = "#C7D2E4"; BG = "#FFFFFF"
...
# defs: soft shadow
s.append('<defs><filter id="sh" x="-20%" y="-20%" width="140%" height="140%">'
         '<feDropShadow dx="0" dy="2" stdDeviation="3" flood-color="#1A2B4A" flood-opacity="0.16"/>'
         '</filter></defs>')

# title
s.append(f'<text x="{W/2}" y="58" text-anchor="middle" font-size="30" font-weight="800" '
         f'fill="{NAVY}" letter-spacing="0.5">Data Model — Fact Constellation</text>')
s.append(f'<text x="{W/2}" y="88" text-anchor="middle" font-size="15" fill="{MUTE}">'
...
def box(x, y, w, h, title, header_fill, rows, header_text="#FFFFFF", rx=12):
    g = [f'<g filter="url(#sh)">']
    g.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{rx}" fill="white" '
             f'stroke="{LINE}" stroke-width="1.5"/>')
    g.append(f'<path d="M{x+1} {y+30} v-18 a12 12 0 0 1 12 -12 h{w-26} a12 12 0 0 1 12 12 v18 Z" '
             f'fill="{header_fill}"/>')
    g.append(f'<text x="{x+16} " y="{y+20}" font-size="15" font-weight="700" fill="{header_text}">{esc(title)}</text>')
    yy = y + 50
    for r, col in rows:
...
# ---- dimensions (right) ----
s.append(box(DIMX, 70, DIMW, 96, "dim_lga  ·  SCD2", BLUE,
             [("lga_sk (PK) · lga_code", INK), ("lga_name · lga_type · is_current", MUTE)]))
s.append(box(DIMX, 210, DIMW, 150, "dim_suburb  ·  SCD2", BLUE,
             [("suburb_sk (PK) · sal_code (BK)", INK),
              ("suburb_name · postcode", MUTE),
              ("lga_sk (FK) · region", MUTE),
              ("asgs_edition · is_current", MUTE)]))
s.append(box(DIMX, 430, DIMW, 120, "dim_year  ·  Type 1", TEAL,
             [("year_sk (PK) · year", INK), ("decade · is_census_year", MUTE)]))
s.append(box(DIMX, 620, DIMW, 120, "dim_geo_quality  ·  Type 1", TEAL,
...
for name, fy, qmap in facts:
    s.append(box(FX, fy, FW, FH, name, NAVY,
                 [("FK: suburb_sk · year_sk · geo_quality_sk", INK),
                  ("\u2192 " + qmap, MUTE)]))
# bridge
bn, by = bridge
s.append(box(FX, by, FW, FH, bn + "  ·  bridge", VIOLET,
...
for key, (bx, col, branch_y) in buses.items():
    ...
    s.append(f'<polygon points="{DIMX-9},{branch_y-5} {DIMX},{branch_y} {DIMX-9},{branch_y+5}" fill="{col}"/>')
    # key label at top of bus
    s.append(f'<text x="{bx}" y="{y0-8}" text-anchor="middle" font-size="11.5" '
...
# ---- legend ----
ly = 880
items = [("SCD2 dimension", BLUE), ("Type 1 dimension", TEAL), ("Fact (grain: suburb × year)", NAVY),
         ("Bridge", VIOLET)]
...
for label, col in items:
    ...
    x += 40 + len(label) * 7.6
# bus key legend
s.append(f'<text x="70" y="{ly+44}" font-size="12.5" fill="{MUTE}">'
...
s.append('</svg>')
open("docs/data-model/er-fact-constellation.svg", "w").write("\n".join(s))
print("wrote docs/data-model/er-fact-constellation.svg")
